...
def weekly_demand_forecast_tft_or_fallback(
    ...
    """TFT があれば週次予測。無い場合（Cloud 等で .ckpt 未同梱）は価格弾力性モデルで同じ列形の DataFrame を返す。"""
...
LANG = {
    "ja": {
        ...
        "tft_unavailable_title": "TFT チェックポイントが見つかりません",
        "tft_unavailable_body": "プライシング・発注の「AI予測」は、価格弾力性モデルで近似表示しています。本番では `models/tft_best_model.ckpt` をデプロイに含めるか、環境変数 `TFT_MODEL_PATH` でパスを指定してください。",
    },
    "en": {
        "title":           "🏪 Retail Decision Engine",
        ...
        "tft_unavailable_title": "TFT checkpoint not found",
        "tft_unavailable_body": "Pricing / order “AI forecast” uses the elasticity model as a fallback. For production, include `models/tft_best_model.ckpt` in the deploy or set `TFT_MODEL_PATH`.",
    },
}
